...
function callback_playerkilled( einflictor, attacker, idamage, smeansofdeath, weapon, vdir, shitloc, psoffsettime, deathanimduration, enteredresurrect = 0 )
{
    ...
    if ( isplayer( attacker ) && isdefined( attacker.pers ) )
    {
        if ( attacker.pers[ #"hash_49e7469988944ecf" ] === 1 )
        {
            ...
            {
                scoreevents::processscoreevent( #"battle_command_command_combo", attacker, self, weapon );
...
// Size: 0x3fc
function private update_weapon_stats( attacker, weapon, smeansofdeath, inflictor )
{
...
function private function_48a1200f( einflictor, attacker, weapon, lpattackteam )
{
    ...
    if ( isdefined( self.attackers ) )
    {
        for ( j = 0; j < self.attackers.size ; j++ )
        {
            ...
            {
                function_92d1707f( #"dlog_event_mpassists", { #gametime:function_f8d53445(), #assistspawnid:getplayerspawnid( player ), #assistspecialist:function_b14806c6( player player_role::get(), currentsessionmode() ), #assistweapon:player.currentweapon.name } );
            }
