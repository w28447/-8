...
function function_72ba0df6( einflictor, attacker, idamage, smeansofdeath, weapon, vdir, shitloc, psoffsettime, deathanimduration )
{
    ...
    if ( level.var_ad7c0539 === 2 && self.team === #"axis" )
    {
        ...
        {
            level.var_3cdb14a8--;
            level.players[ 0 ] ct_utils::ingame_objective_set_points( level.var_3cdb14a8 );
        }
...
function function_45f0ba8d()
{
    ...
    while ( level.var_f9242904 )
    {
        ...
        level.players[ 0 ] ct_utils::ingame_objective_set_points( level.var_37591676 );
...
function function_779e90b6()
{
    ...
    if ( isdefined( queryresults ) && isdefined( queryresults.data ) )
    {
        for ( i = 0; i < 4 ; i++ )
        {
            if ( isdefined( queryresults.data[ i ] ) )
            {
                var_d90eb55d[ i ] = queryresults.data[ i ].origin;
                continue;
            }
            ...
            if ( var_d9fa3a2c <= n_ammo_start && !( isdefined( self.b_reloaded ) && self.b_reloaded ) )
            {
                ...
                while ( !( isdefined( self.b_reloaded ) && self.b_reloaded ) )
                {
                    ...
                    if ( isdefined( queryresults ) && isdefined( queryresults.data ) )
                    {
                        for ( i = 0; i < 4 ; i++ )
                        {
                            var_d90eb55d[ var_d90eb55d.size ] = queryresults.data[ i ].origin;
                        }
                    }
...
{
    self endon( #"death" );
    queryresults = positionquery_source_navigation( self.origin, 48, 156, 8, 32, 1 );
    var_d90eb55d = [];
    ...
    if ( isdefined( queryresults ) && isdefined( queryresults.data ) )
    {
        for ( i = 0; i < 4 ; i++ )
        {
            if ( isdefined( var_d90eb55d ) && isdefined( queryresults.data[ i ] ) )
            {
                var_d90eb55d[ var_d90eb55d.size ] = queryresults.data[ i ].origin;
            }
        }
