...
function init_quests()
{
    ...
    zm_sq::register( #"hells_redeemer", #"hash_42a50f35aa200869", #"hash_574003386f679990", &function_bd0aa04e, &function_13ebfa5e );
    ...
    zm_sq::register( #"laundry_day", #"1", #"laundry_day_quest", &laundry_day, &laundry_day_cleanup );
...
{
    a_str_powerups = array( "double_points", "full_ammo", "insta_kill" );
    
    if ( !zm_custom::function_901b751c( "zmPowerupDouble" ) )
    {
        arrayremovevalue( a_str_powerups, "double_points" );
    }
    
    if ( !zm_custom::function_901b751c( "zmPowerupMaxAmmo" ) )
    {
        arrayremovevalue( a_str_powerups, "full_ammo" );
    }
    
    if ( !zm_custom::function_901b751c( "zmPowerupInstakill" ) )
    {
        arrayremovevalue( a_str_powerups, "insta_kill" );
    }
    
    if ( a_str_powerups.size == 0 || zm_utility::is_standard() )
    {
        return;
    }
    else if ( a_str_powerups.size == 1 )
    {
        str_value = a_str_powerups[ 0 ];
        
        if ( !isdefined( a_str_powerups ) )
        {
            a_str_powerups = [];
        }
        else if ( !isarray( a_str_powerups ) )
        {
            a_str_powerups = array( a_str_powerups );
        }
        
        a_str_powerups[ a_str_powerups.size ] = str_value;
    }
    
    a_str_powerups = array::randomize( a_str_powerups );
    
    for ( i = 1; i <= 2 ; i++ )
    {
        s_power_up = struct::get( "powerup_cell_" + i, "targetname" );
        s_power_up.var_ce471a57 = a_str_powerups[ i - 1 ];
        var_74e47e53 = level.zombie_powerups[ a_str_powerups[ i - 1 ] ].model_name;
    ...
    while ( isdefined( mdl_lock ) )
    {
        ...
        while ( isdefined( e_tomahawk ) )
        {
            if ( distancesquared( e_tomahawk.origin, mdl_lock.origin ) < 10000 && isdefined( mdl_lock.var_1e4fbc72 ) && mdl_lock.var_1e4fbc72 && !isdefined( mdl_lock.var_3d3748c4 ) && !self hasweapon( getweapon( #"zhield_spectral_dw_upgraded" ) ) )
            {
                ...
                level notify( #"lock_box_captured" );
                continue;
            }
...
function function_1877cfe4( e_tomahawk )
{
    ...
    a_str_allowed_zones = array( "zone_catwalk_03", "zone_catwalk_04" );
    
    while ( true )
    {
        str_zone = self zm_zonemgr::get_player_zone();
        
        if ( !isdefined( str_zone ) || !array::contains( a_str_allowed_zones, str_zone ) )
        {
            /#
...
function function_dab3dcc3()
{
    ...
    for ( var_c6b58daa = array::random( var_64de1b5a ); isdefined( var_c6b58daa.e_owner ) ; var_c6b58daa = array::random( var_64de1b5a ) )
    {
    }
...
function function_90dac8fa()
{
    ...
    while ( true )
    {
        ...
        foreach ( e_player in level.activeplayers )
        {
            ...
            {
                for ( var_3c29eed1 = array::random( var_e6a69e36 ); !zm_utility::is_point_inside_enabled_zone( var_3c29eed1.origin ) ; var_3c29eed1 = array::random( var_e6a69e36 ) )
                {
                }
...
function function_3c089853( e_player )
{
    ...
    while ( true )
    {
        ...
        foreach ( var_d72f7274 in var_8ad4ec11 )
        {
            ...
            {
                s_next_location = var_d72f7274;
                break;
            }
        }
        
        if ( isdefined( s_next_location ) )
        {
            self lookatpos( s_next_location.origin );
            self thread function_4d8d3da5( e_player );
            self setgoal( s_next_location.origin, 1 );
            self function_ac75d5bf( e_player, s_next_location );
        }
...
function function_bc827f0d( current_zone, e_dog )
{
    ...
    while ( true )
    {
        ...
        if ( !isdefined( e_dog.s_last_location ) )
        {
            break;
        }
        else if ( e_dog.s_last_location.script_string == str_next_location )
        {
            continue;
        }
...
// Size: 0x8e
function function_ac75d5bf( e_player, s_next_location )
{
    self endon( #"death" );
    self waittill( #"goal" );
    self.s_last_location = s_next_location;
...
function function_c6472238( e_player )
{
    ...
    while ( true )
    {
        ...
        {
            self notify( #"player_too_close_runaway" );
        }
...
function function_442fe4d3()
{
    ...
    while ( true )
    {
        ...
        {
            a_w_allowed_weapons = array( getweapon( #"zhield_spectral_dw" ), getweapon( #"zhield_spectral_dw_upgraded" ), getweapon( #"zhield_spectral_turret" ), getweapon( #"zhield_spectral_turret_upgraded" ) );
            w_current = s_result.activator.currentweapon;
            
            if ( array::contains( a_w_allowed_weapons, w_current ) )
            {
...
function function_27c26acb()
{
    ...
    level waittill( #"walnut_teleporter_ready" );
...
function private function_c8220768()
{
    ...
    self endon( #"disconnect" );
    level endon( #"walnut_teleporter_ready" );
    self waittill( #"done_speaking" );
    level notify( #"walnut_teleporter_ready" );
}
...
function function_22927328( s_params )
{
    ...
    var_5d138a8c = array( level.hero_weapon[ #"flamethrower" ][ 1 ], level.hero_weapon[ #"flamethrower" ][ 2 ], level.hero_weapon[ #"gravityspikes" ][ 1 ], level.hero_weapon[ #"gravityspikes" ][ 2 ], level.hero_weapon[ #"katana" ][ 1 ], level.hero_weapon[ #"katana" ][ 2 ], level.hero_weapon[ #"minigun" ][ 1 ], level.hero_weapon[ #"minigun" ][ 2 ], getweapon( #"hero_minigun_t8_lv2_Grenade" ) );
...
// Size: 0x6c
function private setup_audio_logs()
{
    /#
...
function private audio_log()
{
    ...
    if ( self.e_activator stats::get_stat_global( #"darkops_escape_ee" ) )
    {
