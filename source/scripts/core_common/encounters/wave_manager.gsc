        ...
        while ( true )
        {
            ...
            if ( n_wave_manager_id != -1 )
            {
                if ( isdefined( level.var_ca74a4bc[ n_wave_manager_id ] ) )
                {
                    ...
                    /#
                        str_text = "<dev string:x175>" + n_total_count + "<dev string:x189>" + n_active_count + "<dev string:x68>";
                        var_c708e6e1 += n_y_offset;
                        debug2dtext( ( 800, var_c708e6e1, 0 ), str_text, ( 0, 1, 0 ), 1, ( 0, 0, 0 ), 0.9, 1, 1 );
                        var_c708e6e1 += n_y_offset;
                        debug2dtext( ( 800, var_c708e6e1, 0 ), "<dev string:xf4>", ( 0, 1, 1 ), 1, ( 0, 0, 0 ), 0.9, 1, 1 );
...
function private function_ff49692b( var_dcd6c23, ai )
{
    ...
    if ( isdefined( var_dcd6c23.var_23802722 ) )
    {
        ...
        {
            if ( isdefined( var_e8332bc1.func_on_spawn ) )
            {
                util::single_thread_argarray( ai, var_e8332bc1.func_on_spawn, var_e8332bc1.a_params );
            }
        }
    }
}
...
function private function_68ed489( var_dcd6c23 )
{
    ...
    foreach ( var_c21b798e in var_dcd6c23.var_376c2c29 )
    {
        var_c21b798e.m_a_ai = array::remove_dead( var_c21b798e.m_a_ai );
    ...
    while ( true )
    {
        ...
        while ( var_6a60a2e6 < n_group_size )
        {
            ...
            if ( !isdefined( var_549391b8 ) || !isdefined( var_549391b8[ #"spawner" ] ) || var_549391b8[ #"spawner" ].count < 1 && !( isdefined( var_549391b8[ #"spawner" ].spawnflags ) && ( var_549391b8[ #"spawner" ].spawnflags & 64 ) == 64 ) )
            {
                ...
                {
                    /#
                        println( "<dev string:x3f9>" + str_ai_type + "<dev string:x423>" + var_dcd6c23.var_556afb3d + "<dev string:x437>" );
                        iprintln( "<dev string:x3f9>" + str_ai_type + "<dev string:x423>" + var_dcd6c23.var_556afb3d + "<dev string:x437>" );
                    #/
...
// Size: 0xf8
function start( kvp, func_on_spawn, ... )
{
    ...
    foreach ( s_wave_manager in a_s_wave_managers )
    {
        ...
        var_e8332bc1.func_on_spawn = func_on_spawn;
...
// Size: 0xa2
function function_be3a34f( var_b6ee6116, str_team, b_looping = 0, str_spawner_targets, func_on_spawn, ... )
{
    var_e8332bc1 = new class_8e39177();
    var_e8332bc1.func_on_spawn = func_on_spawn;
    var_e8332bc1.a_params = vararg;
...
function function_1c556906( kvp, str_spawner_targetname )
{
    ...
    foreach ( var_dcd6c23 in var_fed53aae )
    {
        ...
        assert( var_91504a05, "<dev string:x4fa>" + str_spawner_targetname );
...
// Size: 0x37a
function add_spawn_function( kvp, func_on_spawn, ... )
{
    assert( isdefined( func_on_spawn ) );
...
// Size: 0x2ee
function remove_spawn_function( kvp, func_on_spawn )
{
    assert( isdefined( func_on_spawn ) );
    a_s_wave_managers = self wave_manager_sys::function_63e08195( kvp, 0 );
    ...
    if ( a_s_wave_managers.size )
    {
        foreach ( s_wave_manager in a_s_wave_managers )
        {
            if ( isdefined( s_wave_manager.var_dcd6c23 ) )
            {
                ...
                foreach ( var_e8332bc1 in var_dcd6c23.var_23802722 )
                {
                    if ( var_e8332bc1.func_on_spawn === func_on_spawn )
                    {
