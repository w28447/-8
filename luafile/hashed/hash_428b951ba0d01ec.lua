...
CoD.CharacterSelection_StandardFrameWidget.new = function ( f1_arg0, f1_arg1, f1_arg2, f1_arg3, f1_arg4, f1_arg5, f1_arg6, f1_arg7, f1_arg8, f1_arg9 )
	...
	f1_arg0:AddButtonCallbackFunction( Characters, f1_arg1, Enum.LUIButton[0x755DA1E2E7C263F], "ui_confirm", function ( element, menu, controller, model )
		if not CoD.ModelUtility.IsSelfModelValueTrue( element, controller, "purchasable" ) and not CoD.ModelUtility.IsSelfModelValueTrue( element, controller, "disabled" ) and not IsGameTrial() and CoD.PlayerRoleUtility[0x7841254A6DA6220]( element, controller ) then
		...
		elseif IsElementInState( element, "Purchasable" ) and not IsGameTrial() and CoD.StoreUtility.IsStoreEnabled( controller ) then
			OpenStore( self, element, controller, "", menu )
			return true
	...
	f1_arg0:AddButtonCallbackFunction( Characters, f1_arg1, Enum.LUIButton[0xE6DB407A2AF8B09], "ui_contextual_1", function ( element, menu, controller, model )
		...
		if IsGamepad( controller ) and not CoD.ModelUtility.IsSelfModelValueTrue( element, controller, "disabled" ) and not IsGameTrial() and CoD.ModelUtility.IsSelfModelValueTrue( element, controller, "hasCustomization" ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton[0xE6DB407A2AF8B09], "menu/personalize_character", nil, "ui_contextual_1" )
			return true
		elseif not IsGamepad( controller ) and not CoD.ModelUtility.IsSelfModelValueTrue( element, controller, "disabled" ) and not IsGameTrial() and CoD.ModelUtility.IsSelfModelValueTrue( element, controller, "hasCustomization" ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton[0xE6DB407A2AF8B09], "menu/personalize_character", Enum[0xBEBDBAEEB3ECCCA][0xB6372335C630AD3], "ui_contextual_1" )
			return true
		else
			return false
		end
	end, false )
	Characters:AddContextualMenuAction( f1_arg0, f1_arg1, "menu/personalize_character", function ( f16_arg0, f16_arg1, f16_arg2, f16_arg3 )
	...
	CharacterText:setText( LocalizeToUpperString( "wz/characters" ) )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				return CoD.ModelUtility.IsSelfModelValueEqualToEnum( element, f1_arg1, "mode", Enum.eModes.mode_zombies )
			end
		}
	} )
	...
	f1_arg0:AddButtonCallbackFunction( self, f1_arg1, Enum.LUIButton[0x29E5695FF1401AD], nil, function ( element, menu, controller, model )
		...
		if CoD.ModelUtility.IsGlobalDataSourceModelValueGreaterThan( controller, "CharacterBreadcrumbs", "breadcrumbCount", 0 ) and not IsPC() then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton[0x29E5695FF1401AD], 0x5619D8212EDA599, nil, nil )
			return true
		else
			return false
		end
