...
LobbyVM.LogGlobalData = function ()
	...
	local f9_local3 = {
		playlist_version = Engine[0xB15BEE2BE53060A](),
		protocol_version = Engine[0xBDC89A97B6CE16D](),
	...
	f9_local3.build_changelist = Engine.GetBuildIntField( Enum.BuildIntField[0xBABF31AFD223EE0] )
	f9_local3.build_machine = Engine.GetBuildStringField( Enum.BuildStringField[0xDD83FC85A90B9DE] )
	f9_local3.build_time = Engine.GetBuildStringField( Enum.BuildStringField[0xF4435CE5FA728B4] )
	f9_local3.build_type = Engine.GetBuildStringField( Enum.BuildStringField[0x64E89CE128A661D] )
...
LobbyVM.OnDisconnect = function ( f17_arg0 )
	...
	elseif f17_local4 == Enum.LobbyDisconnectClient[0x6EF5F11FAA609FC] then
		f17_local6 = false
		f17_local7 = Engine[0xF9F1239CFD921FE]( "menu/notice" )
		f17_local8 = Engine[0xF9F1239CFD921FE]( "platform/partychatnotallowed" )
		Dvar[0xE21BBE3614F731D]:set( false )
	elseif f17_local4 == Enum.LobbyDisconnectClient[0x75E935EEE527F8B] then
		f17_local6 = false
		f17_local7 = Engine[0xF9F1239CFD921FE]( "exe/kicked_from_party_warning_title" )
		f17_local8 = Engine[0xF9F1239CFD921FE]( "exe/kicked_from_party_warning" )
	...
	elseif f17_local4 == Enum.LobbyDisconnectClient[0x18DF95700618225] then
		if LobbyVM.ShouldShowContentChangedMessage( f17_local5, f17_local1 ) then
			...
			Engine[0xBC42C678E40DBE8]( Enum.errorcode[0x100911D2B38A4EF], Engine[0xF9F1239CFD921FE]( "platform/missingmap" ) )
		end
	...
	elseif f17_local4 == Enum.LobbyDisconnectClient[0xB901B6DF0CD7657] then
		f17_local6 = false
		f17_local7 = Engine[0xF9F1239CFD921FE]( "exe/kicked_from_party_warning_title" )
		f17_local8 = Engine[0xF9F1239CFD921FE]( "menu/korea_15plus_blocked_gamemode" )
		if f17_local1 == Enum.LobbyType[0xA1647599284110] and Engine.IsInGame() == true then
			f17_local9 = Lobby.Process.HostLeftNoMigrationCreatePrivateLobby( f17_local5, f17_local2, Engine[0x29B25E8DA873863]( Enum.LobbyModule[0xC46B73E8E18BA2], f17_local1 ) )
		end
...
LobbyVM.OnUILoad = function ( f19_arg0 )
	...
	elseif false == Engine[0xE39F1F30B306065]() then
		...
		if f19_local2.id == f19_local3.id then
			Engine[0x41D81D6B58AAF3F]( Enum.LobbyType[0xA1647599284110], f19_local3.maxclients )
...
LobbyVM.OnLobbyHostLeftNoMigration = function ( f30_arg0 )
	...
	Engine[0xDE279ECDDDD966]( f30_local0, 0x7ED422C1906DDFB, {
		...
		lobby_main_mode = f30_arg0.lobbyMainMode,
		lobby_network_mode = f30_arg0.lobbyNetworkMode,
		max_clients = f30_arg0.maxClients,
		is_game_lobby_active = f30_arg0.isGameLobbyActive,
		is_private_host = f30_arg0.isPrivateHost
	} )
end
...
LobbyVM.OnLobbySettings = function ( f48_arg0 )
	...
	Engine.PrintInfo( Enum[0x7A63DCD561B0FA8][0x59962D5EF982597], "Applying settings for menu: " .. f48_local1.name .. ".\n" )
	if not Engine.IsInventoryBusy( f48_local0 ) and Engine[0xCB675CA7856DA25]() and f48_local1.mainmode ~= Enum.LobbyMainMode[0xD5FBB8D74AC6D62] and f48_local1.mainmode ~= Enum.LobbyMainMode[0x78C124999125C42] then
		LuaUtils.SafeComError( Enum.errorcode[0x7039D0A1017FE92], "menu/join_result_full_version_required_title" )
	end
	if not Engine.IsInventoryBusy( f48_local0 ) and f48_local1.mainmode == Enum.LobbyMainMode[0x79D01499920B292] then
		if Engine[0xA8FBC7AC4C3F3A6]() then
			LuaUtils.SafeComError( Enum.errorcode[0x7039D0A1017FE92], "menu/korea_15plus_blocked_gamemode" )
	...
	Engine[0x41D81D6B58AAF3F]( f48_local1.LobbyType, f48_local1.maxclients )
end
...
LobbyVM.LobbyStatusUpdate = function ( f69_arg0 )
	...
	if f69_local4 == 1 then
		...
		if f69_local6 == 1 then
			f69_local5 = Engine[0xF9F1239CFD921FE]( "menu/sessions_search_stage_single" )
		else
			f69_local5 = Engine[0xF9F1239CFD921FE]( "menu/sessions_search_stage", f69_local6 )
		end
		f69_local1:set( f69_local5 )
		f69_local2:set( "" )
		f69_local3:set( "" )
	elseif f69_local4 == 2 then
		f69_local2:set( Engine[0xF9F1239CFD921FE]( "menu/sessions_qos_progress", f69_arg0.contactedResults, f69_arg0.numResults ) )
	elseif f69_local4 == 3 then
		f69_local3:set( Engine[0xF9F1239CFD921FE]( "menu/sessions_joining", f69_arg0.joiningCurHost, f69_arg0.joiningNumHosts ) )
	end
end
...
LobbyVM.OnGameLobbyGameServerDataUpdate = function ( f74_arg0 )
	...
	if f74_local0.LobbyMode == Enum.LobbyMode[0xBB5FD8AEFC4D4B9] or f74_local0.LobbyMode == Enum.LobbyMode[0xD42D003CEEA3F87] then
		...
		if f74_local1 then
			...
			Engine.SetModelValue( Engine.CreateModel( f74_local3, "kickerText" ), "" )
		end
	end
...
LobbyVM.OnPrivateLobbyServerDataUpdate = function ( f75_arg0 )
	...
	if f75_local0.LobbyMode == Enum.LobbyMode[0xBB5FD8AEFC4D4B9] or f75_local0.LobbyMode == Enum.LobbyMode[0xD42D003CEEA3F87] then
		...
		if f75_local2 then
			...
			Engine.SetModelValue( Engine.CreateModel( f75_local4, "kickerText" ), "" )
		end
	end
end
...
LobbyVM.GetRecentItemTags = function ( f84_arg0, f84_arg1 )
	local f84_local0 = Engine[0xF9DEF4016ECF1FB]( f84_arg0, Enum.eModes.mode_multiplayer, 0, Engine[0xE3E5F674ECC4A61]( f84_arg0, Enum.eModes.mode_multiplayer ) )
...
LobbyVM.CheckSpecialPlaylistRules = function ( f88_arg0 )
	if Dvar[0xE21BBE3614F731D]:get() == true then
		if Engine[0x4160EF6B98737EF]() and not Engine[0x1B09567078C4A6F]() then
			Dvar[0xE21BBE3614F731D]:set( false )
			Engine[0xBC42C678E40DBE8]( Enum.errorcode[0x100911D2B38A4EF], Engine[0xF9F1239CFD921FE]( "platform/partychatnotallowed" ) )
		end
		...
		for f88_local4, f88_local5 in ipairs( f88_local0.sessionClients ) do
			if f88_local5.isInPlatformPartyChat == true then
				Engine[0x3C379E096BB7570]( f88_arg0, Enum.LobbyType[0x92676CF5B6FCD43], f88_local5.xuid, Enum.LobbyDisconnectClient[0x6EF5F11FAA609FC], "platform/partychatnotallowed" )
			end
		end
	end
end
	...
	elseif (Engine[0xE39F1F30B306065]() or CoDShared.IsLobbyMode( Enum.LobbyMode[0xBB5FD8AEFC4D4B9] )) and Dvar[0xE21BBE3614F731D]:get() and Engine[0x3E68E350BEFE50D]( Enum.LobbyModule[0x98EA1BB7164D103], Enum.LobbyType[0x92676CF5B6FCD43] ) then
		...
		for f89_local5, f89_local6 in ipairs( f89_local0.sessionClients ) do
			if f89_local6.isInPlatformPartyChat == true then
				Engine[0x3C379E096BB7570]( f89_local1, Enum.LobbyType[0x92676CF5B6FCD43], f89_local6.xuid, Enum.LobbyDisconnectClient[0x6EF5F11FAA609FC], "platform/partychatnotallowed" )
			end
		end
	end
end
